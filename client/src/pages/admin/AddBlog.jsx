...
const AddBlog = () => {
  ...
  const onSubmitHandler=async(e)=>{
    const blog = {title,subTitle,category,description:quillRef.current.root.innerHTML,isPublished};
    const formData = new FormData();
    formData.append('blog',JSON.stringify(blog));
    if (image) {
      formData.append('image', image);  // 'image' matches your multer field name
    }
    e.preventDefault();
    try {
      setIsAdding(true);
    ...
    <form onSubmit={onSubmitHandler} className='flex-1 bg-blue-50/50 text-gray-600 h-full overflow-scroll'>
      <div className='bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 shadow rounded'>
        ...
        <div className='max-w-lg h-74 pb-16 sm:pb-10 pt-2 relative'>
          ...
          <button disabled={loading} type='button' className='absolute bottom-1 right-2 ml-2 text-xs text-white bg-black/70 px-4 py-1.5 rounded hover:underline cursor-pointer' 
          onClick={generateContent}>{loading?"Loading...":"Generate with AI"}</button>
        </div>
